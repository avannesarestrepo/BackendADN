...
public class ConsultaControladorPropietarioTest {
    ...
    void consultarPropietarioPorIdExitosa() throws Exception {
        ComandoPropietario comandoPropietarioTestDataBuilder = new ComandoPropietarioTestDataBuilder()
                .propietarioExistentePorDefecto().build();
    ...
    @Test
    void consultarPropietariosExitosa() throws Exception {
        ComandoPropietario comandoPropietarioTestDataBuilder = new ComandoPropietarioTestDataBuilder()
                .propietarioExistentePorDefecto().build();
        ...
        mockMvc.perform(get("/propietario")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().is2xxSuccessful())
                .andExpect(jsonPath("$[0].id", is(comandoPropietarioTestDataBuilder.getId().intValue())))
                .andExpect(jsonPath("$[0].tipoDocumento", is(comandoPropietarioTestDataBuilder.getTipoDocumento().toString())))
                .andExpect(jsonPath("$[0].documento", is(comandoPropietarioTestDataBuilder.getDocumento())))
                .andExpect(jsonPath("$[0].nombreCompleto", is(comandoPropietarioTestDataBuilder.getNombreCompleto())))
                .andExpect(jsonPath("$[0].email", is(comandoPropietarioTestDataBuilder.getEmail())))
                .andExpect(jsonPath("$[0].telefono", is(comandoPropietarioTestDataBuilder.getTelefono())))
                .andReturn();
    }
}
